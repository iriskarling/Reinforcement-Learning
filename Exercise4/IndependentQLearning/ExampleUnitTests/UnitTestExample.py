...
if __name__ == '__main__':
	...
	for epsNum in range(numEpisodes):
		...
		while status[0]=="IN_GAME":
			...
			for agentIdx in range(2):
				agents[agentIdx].setExperience(agents[agentIdx].toStateRepresentation(stateCopies[agentIdx]), actions[agentIdx], reward, 
					status, agents[agentIdx].toStateRepresentation(nextObservation))
				learnResult = agents[agentIdx].learn()

				if not np.isclose(learnResult, float(outputData[timeSteps+1][agentIdx+1]), atol=1e-5):
					print("Wrong Output")
					exit()
