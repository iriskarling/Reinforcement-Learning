	...
	for epsNum in range(numEpisodes):
		...
		while status[0]=="IN_GAME":
			...
			for agentIdx in range(2):
				...
				if not np.isclose(learnRes, float(outputData[timeSteps+1][agentIdx+1]), atol=1e-4):
				...
				for idx in range(len(averageUpdateOut)):
					if not np.isclose(averageUpdateOut[idx], correctAvg[idx], atol=1e-4):
				...
				for idx in range(len(policyOut)):
					if not np.isclose(policyOut[idx], correctPol[idx], atol=1e-4):
